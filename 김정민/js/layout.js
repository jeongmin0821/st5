...
function top_category(){
    ...
    jQuery('#header .xans-layout-category > ul > li').mouseenter(function(){
        jQuery(".sub-category-box").css('display', 'block'); // 모든 2depth 표시
        jQuery("#header .bg-d2").addClass('on'); // 2depth 배경 표시
    ...
    jQuery('#header').mouseleave(function(){
        jQuery(".sub-category-box").css('display', 'none'); // 모든 2depth 숨김
        jQuery("#header .bg-d2").removeClass('on'); // 2depth 배경 숨김
        jQuery("#layer_shadow").removeClass('on');
        jQuery('body').removeClass('not_scroll').unbind('scroll touchmove mousewheel');
    });
